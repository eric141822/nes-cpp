...
namespace NES
{
    ...
    std::string trace(CPU &cpu)
    {
        ...
        u16 addr = (ops.mode == AddressingMode::Immediate || ops.mode == AddressingMode::NoneAddressing) ? 0 : cpu.get_abs_address(ops.mode, begin+1);
        u8 stored_value = (ops.mode == AddressingMode::Immediate || ops.mode == AddressingMode::NoneAddressing) ? 0 : cpu.mem_read(addr);
        ...
        case 2:
        {
            u8 address = cpu.mem_read(begin + 1); // get address of operand.
            ...
            {
                tmp = fmt::format("(${:02X},X) @ {:02X} = {:04X} = {:02X}", address, address + static_cast<u16>(cpu.register_x), addr, stored_value);
                break;
            }
            case AddressingMode::IndirectY:
            {
                tmp = fmt::format("(${:02X}),Y = {:04X} @ {:04X} = {:02X}", address, addr - static_cast<u16>(cpu.register_y), addr, stored_value);
                break;
            }
            case AddressingMode::NoneAddressing:
            {
                u16 jmp_addr = begin + 2 + static_cast<u8>(address);
        ...
        case 3:
        {
            u8 address_lo = cpu.mem_read(begin + 1);
            u8 address_hi = cpu.mem_read(begin + 2);
            dump.push_back(address_lo);
            dump.push_back(address_hi);
            u16 address = cpu.mem_read_u16(begin + 1);
            switch (ops.mode)
            {
            case AddressingMode::NoneAddressing:
            {
                ...
                {
                    // indirect jmp;
                    u16 jmp_addr = ((address & 0x00ff) == 0x00ff) ? static_cast<u16>(cpu.mem_read(address & 0xff00)) << 8 | static_cast<u16>(cpu.mem_read(address)) : cpu.mem_read_u16(address);
