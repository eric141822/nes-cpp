...
namespace NES
{
    ...
    void CPU::run_with_callback(std::function<void(CPU &)> callback)
    {
        while (true)
        {
            ...
            case 0xD3:
            {
                u8 addr = get_operand_address(opcode.mode);
                u8 data = mem_read(addr);
                data--;
                mem_write(addr, data);
                if (data <= register_a)
                {
                ...
                set_zero_and_negative_flags((register_a - data));
                break;
            }
            ...
            case 0xCB:
            {
                u8 addr = get_operand_address(opcode.mode);
                u8 data = mem_read(addr);
                u8 res = (register_a & register_x) - data;
                if (data <= (register_x & register_a))
                {
                    status |= cpu_flags::CARRY;
                }
                set_zero_and_negative_flags(res);
                register_x = res;
                break;
            }
            ...
            case 0xFC:
            {
                mem_read(get_operand_address(opcode.mode));
                break;
            }
            ...
            case 0xBB:
            {
                ...
                register_x = data;
                stack_pointer = data;
                set_zero_and_negative_flags(data);
                break;
            }
            ...
            case 0x9B:
            {
                u8 data = register_a & register_x;
                stack_pointer = data;
                u16 mem_addr = mem_read_u16(pc) + static_cast<u16>(register_y);
                data = (static_cast<u8>((mem_addr >> 8)) + 1) & stack_pointer;
                mem_write(mem_addr, data);
                break;
            }
            ...
            {
                u8 pos = mem_read(pc);
                u16 mem_addr = mem_read_u16(pos) + static_cast<u16>(register_y);
                u8 data = register_a & register_x & static_cast<u8>((mem_addr >> 8));
                mem_write(mem_addr, data);
                break;
            }
            ...
            {
                u16 mem_addr = mem_read_u16(pc) + static_cast<u16>(register_y);
                u8 data = register_x & (static_cast<u8>((mem_addr >> 8)) + 1);
                mem_write(mem_addr, data);
                break;
            }
            ...
            case 0x9C:
            {
                u16 mem_addr = mem_read_u16(pc) + static_cast<u16>(register_x);
                u8 data = register_y & (static_cast<u8>((mem_addr >> 8)) + 1);
                mem_write(mem_addr, data);
                break;
            }
    ...
    u16 CPU::get_abs_address(AddressingMode mode, u16 begin)
    {
        ...
        case AbsoluteX:
        {
            ...
            u16 addr = base + static_cast<u16>(register_x);
            return addr;
        }
        case AbsoluteY:
        {
            u16 base = mem_read_u16(begin);
            u16 addr = base + static_cast<u16>(register_y);
            return addr;
        }
        case IndirectX:
        {
            ...
            u8 ptr = static_cast<u8>(base) + register_x;
            u8 lo = mem_read(static_cast<u16>(ptr));
            u8 hi = mem_read(static_cast<u16>((ptr + 1)));
            return (static_cast<u16>(hi) << 8) | static_cast<u16>(lo);
        }
        case IndirectY:
        {
            u8 base = mem_read(begin);
            u8 lo = mem_read(static_cast<u16>(base));
            u8 hi = mem_read(static_cast<u16>(base + 1));
            u16 deref_base = (static_cast<u16>(hi) << 8) | static_cast<u16>(lo);
