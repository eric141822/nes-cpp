...
namespace NES
{
    ...
    Rom::Rom(std::vector<u8> raw)
    {
        ...
        this->prg_rom = std::vector<u8>(raw.begin() + prg_rom_start, raw.begin() + prg_rom_start + prg_rom_size);
        this->chr_rom = std::vector<u8>(raw.begin() + chr_rom_start, raw.begin() + chr_rom_start + chr_rom_size);
